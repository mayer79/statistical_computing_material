...
# predictions equal mean(diamonds$price)
...
- **RMSE:** The RMSE is 1549 USD. This means that residuals (= prediction errors) are typically around 1549 USD. More specifically, using the empirical rule (and assuming normality), about $68\%$ of the observed values fall within $\pm 1549$ USD of the predictions.
...
ggplot(
  to_plot,
  aes(x = carat, y = prediction, group = color, color = color)
) +
  geom_line() +
  geom_point() +
  facet_wrap(~model)
...
- **RMSE:** According to the empirical rule, approximately 68% of the observations fall within +-40% of the predictions.
...
- **RMSE:** According to the empirical rule, approximately 68% of the observed values fall within +-26% of the predictions.
...
fit <- lm(
  log(price) ~ log(carat) + color + cut + clarity,
  data = diamonds
)
summary(fit)
```
...
- **RMSE:** According to the empirical rule, approximately 68% of the observed values fall within +-13% of the predictions.
...
In the following example, we will use Parquet for data storage, Arrow to load the data, R's data.table for preprocessing, and H2O to fit a linear regression model. We deliberately combine different technologies to showcase their respective strengths. In practice, however, it is preferable to build workflows with less technology breaks.

All four technologies share three important characteristics: they are free to use, open source, and, at the time of writing, state of the art.
...
- *Run-length encoding:* Multiple occurrences of a value are replaced by the value and the number of repetitions. Example: if "3.1", occurs 100 times after each other, only two values are needed for reconstruction: "3.1" and "100".
- *Standard compression:* Resulting values (within column chunks) are compressed using standard compression algorithms like Snappy or Zstd.
...
Parquet files can be read via the (Py)Arrow library in R or Python. Many other technologies (Apache Spark, DuckDB, Polars) also support working with Parquet files. We will get to know some of them later.
...
Computation time refers to a single run on an Intel i7 CPU with 14 physical cores.
...
df[
  ...
  `:=`(
    ...
    weekday = factor(data.table::wday(tpep_pickup_datetime)), # 1 = Su
...
# Plot only subset
ggplot(
  df[sample(df[, .N], 1e4)], aes(log_duration, log_distance)
) +
...
# Fit model with 68 parameters via lm(): 33 seconds
system.time(
...
# Copy data to cluster. This step is not ideal. Better: Store prepared data
# as parquet file, and load that directly into h2o.
...
# 1  Intercept     1.725249  0.001012 1705.107619 0.000000
# 2 pu_loc.107    -0.049542  0.001077  -45.998056 0.000000
# 3 pu_loc.113    -0.032204  0.001189  -27.089905 0.000000
# 4 pu_loc.114    -0.005347  0.001258   -4.249429 0.000021
# 5 pu_loc.132    -0.439152  0.001141 -384.927728 0.000000
#
# ---
#           names coefficients std_error     z_value  p_value
# 63    weekday.3     0.113730  0.000440  258.566151 0.000000
# 64    weekday.4     0.136778  0.000434  315.249577 0.000000
# 65    weekday.5     0.169123  0.000464  364.837805 0.000000
# 66    weekday.6     0.186043  0.000451  412.866598 0.000000
# 67    weekday.7     0.091005  0.000448  203.175671 0.000000
# 68 log_distance     0.740956  0.000148 4992.109899 0.000000
#
...
- Data preparation with "data.table" is fast. Furthermore, it takes only about 0.05 seconds to calculate counts and average durations per pick-up location.
...
6. Bonus: Visit https://pola-rs.github.io/r-polars/ and play with the Polars library in R. Use Polar's lazy mode to approximate the data preparation done with "data.table" above.
