...
The focus of this chapter is on computer-intensive methods to calculate confidence intervals and to perform statistical tests. We will give a short introduction into statistical inference first.
...
Let $\mu = \mathbb E(F)$ denote the true mean of a distribution $F$ with standard deviation $\sigma = \sqrt{\text{Var}(F)} < \infty$. Usually, $F$, $\mu$, and $\sigma$ are all unknown. Furthermore, let $\boldsymbol X = (X_1, \dots, X_n)$ be a sample of independent random variables drawn from $F$.

What can we say about the sample mean $\hat \mu = \frac{1}{n}\sum_{i = 1}^n X_i$ as an estimator of $\mu$?

1. The sample mean $\hat \mu$ is unbiased: 
$$
...
$$
where $\sigma(\hat\mu)$ is the standard deviation of the sample mean. Standardization ensures that the limit distribution is non-trivial.
...
The LLN says that the sample mean approaches the true mean as the sample size grows. We illustrate this by simulating $10^5$ sample means for different sample sizes $n$. The values are all drawn from an exponential distribution with density $f(x) = \lambda e^{-\lambda x}, \ x\ge 0$, with $\lambda = 1/\mu = 1$.
...
# Standardized mean
std_mean <- function(x, mu = 1, sd = mu) {
...
for (n in c(1, 10, 50, 200)) {
  means <- replicate(nsims, std_mean(rexp(n)))
  hist(means, breaks = 99, main = paste("n =", n), col = "orange",
...
- The case $n=1$ simply shows the histogram of a standard exponential variate. It is very asymmetric and non-normal.
- Note that the sum of standard exponentials is Gamma distributed.
...
Let's use again a sample of exponential values to calculate an approximate 95% confidence interval for the true mean $\mu$.
...
Accuracy of confidence intervals can be assessed by comparing the nominal coverage probability $1 - \alpha$ with the *real* coverage probability. The latter is calculated as the expected proportion of intervals containing the true value $\mu$.

#### Example: Accuracy of the CI

In practice, we only have one sample and one confidence interval, so we cannot calculate the real coverage probability and have to trust the nominal value. In our artificial situation, we can use simulation to generate many samples and their confidence intervals and then estimate the real coverage probability.
...
results <- replicate(nsims, ci(rexp(n, rate = 1 / theta))) %>% 
  t() %>% 
  data.frame() %>% 
  mutate(ok = (lci <= theta) & (theta <= uci))
head(results)
...
ggplot(results[1:100, ], aes(x = rank(estimate), y = estimate, color = ok)) +
  ...
  geom_hline(yintercept = theta, color = "darkgray") +
  coord_flip() +
...
**Comments:** The real coverage probability is lower than the stated one. This undesirable property might be a consequence of the skewed data.

## Other estimators

So far, we have been mainly concerned with the sample mean. What about other estimators to estimate other parameters? 
...
According to @efron1993 (p. 47), 25 to 200 Bootstrap samples are usually sufficient to get a stable Bootstrap estimate of the standard error. The following curve indicates differently: We calculate the Bootstrap estimate of standard error for all sequences from 1 to $b$, $20 \le b \le B$ and plot the results:

```{r}
plot_stability <- function(x) {
...
$$
using the Bootstrap estimate of the standard error in place of $\hat \sigma(\hat\theta)$. 
...
The logic for applying the Bootstrap is as before: resample $B$ times with replacement, calculate the $B$ Bootstrap replicates, and then use these $B$ values to calculate standard errors and/or confidence intervals.

There are two ways of how to perform the resampling:

1. By resampling within sample. This keeps the sample sizes fixed.
2. By resampling rows in a dataset with the group represented as a column. (Like this, we turn the multi-group situation into a multivariate situation, see below.)

Option 1 is usually preferred.

### Example: Mean difference

Let us construct two samples from different exponential distributions: One with true mean 2, the other with true mean 1. Imagine the values are weeks of hospital stay, and the two groups receive different treatments. The task is to create a 95% confidence interval for the true mean difference. The classic approach would be to calculate a two-sample Student confidence interval with unequal variances.
...
**Comment:** The two bootstrap intervals are much narrower than the student interval. We do not know which of the three intervals is the best. Since we know the true distributions of x and y in this example, we could again perform a simulation study and calculate the accuracy of the intervals based on the difference between the actual and nominal coverage probabilities. In practice, this is not possible because the true distributions are unknown. Otherwise we wouldn't need statistics, after all!

## Multivariate estimators

By resampling *rows* in a dataset, we can also work with other multivariate estimators such as the Pearson correlation, Kendall's rank correlation or the R-squared of a linear regression. This allows to study and measure associations between two or more variables using the Bootstrap.
...
# Long form
y <- c(y_A, y_B)
g <- rep(c("A", "B"), times = c(m, n))
data.frame(y, g)[18:22, ]

boxplot(y ~ g, col = "chartreuse4", varwidth = TRUE)
...
statistic <- function(y, g) {
  mean_difference <- mean(y[g == "B"]) - mean(y[g == "A"])
  abs(mean_difference)
}

# Calculate approximate null distribution
B <- 1e4
perm_dist <- replicate(B, statistic(y, sample(g)))

# p value
mean(perm_dist >= statistic(y, g))

# Via "coin" package
library(coin)

independence_test(y ~ factor(g), distribution = approximate(nresample = 10000))
```
...
```{r}
successive_pvalues <- cumsum(perm_dist >= statistic(y, g)) / (1:B)
...
if (FALSE) {  # Set to TRUE to rerun
  for (i in 1:nsim) {
    ...
    result[i, "Welch"] <- t.test(y ~ g)$p.value <= 0.05
    result[i, "Student"] <- t.test(y ~ g, var.equal = TRUE)$p.value <= 0.05
    
    # Permutation test with B = 10000
    perm_dist <- replicate(1e4, statistic(y, sample(g)))
    perm_p <- mean(perm_dist >= statistic(y, g))
...
# Long form
y <- c(y1, y2)
g <- rep(c("A", "B"), times = c(length(y1), length(y2)))

# Wilcoxon's rank sum test
wilcox.test(y ~ g)$p.value

# Permutation test
statistic <- function(y, g) {
  r <- rank(y)
  abs(mean(r[g == "B"]) - mean(r[g == "A"]))
...
wilcox_test(y ~ factor(g), distribution = "exact")
```
...
perm_dist <- replicate(1e4, statistic(sample(y), x))
...
n <- length(pre)
y <- c(pre, post)
g <- rep(c("pre", "post"), each = n)
block <- rep(1:n, times = 2)
data.frame(y, g, block)

independence_test(y ~ factor(g) | factor(block), distribution = "exact")
``` 
...
1. In this exercise, we consider 95%-confidence intervals for the true mean of a uniform distribution.
    ...
    c. Use the `plot_stability()` function for the lecture notes to figure out after how many Bootstrap samples the Bootstrap estimate of the standard error would stabilize.
    d. Calculate a standard normal Bootstrap CI and a percentile Bootstrap CI for $\mu$. Compare with the interval from 1a.

2. Consider the two samples $x_1 = 1, 2, \dots, 21$ and $x_2 = 1, 2, \dots, 51$. 
    a. Using resampling within groups to calculate a percentile Bootstrap CI for the true median difference $\text{Med}(x_2) - \text{Med}(x_1)$. Interpret the result.
    b. Calculate an estimate for the parameter of interest and calculate a standard normal Bootstrap CI. Compare the two solutions.
  
3. For the situation in Exercise 1, use simulation to calculate real coverage probabilities for the Student CI and the two types of Bootstrap CIs. What do you observe?
...
4. Here, we study a test on Spearman's rank correlation.
    ...
    you want to show that the true rank correlation is positive.
...
5. In the situation of Exercise 4: Program a simulation study to compare your approach with `stats::cor.test()` regarding...
