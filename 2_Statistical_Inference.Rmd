...
for (n in c(1, 10, 50, 200)) {
  ...
  hist(
    means,
    breaks = 99, main = paste("n =", n), col = "orange",
...
- The case $n=1$ simply shows the histogram corresponding to a shifted exponential distribution. It is very asymmetric and non-normal.
...
By the CLT and if $n$ is not too small,
$$
  P\big(-z_{1-\alpha/2} \le \frac{\hat\mu - \mu}{\sigma(\hat \mu)} \le z_{1 - \alpha/2}\big) \approx 1 - \alpha,
$$
where $z_\beta$ is the $\beta$-quantile of the standard normal distribution. Solving for the unknown parameter $\mu$, we get
$$
  P\big(\hat\mu - z_{1-\alpha/2} \sigma(\hat\mu) \le \mu \le \hat \mu + z_{1-\alpha/2}\sigma(\hat\mu)\big) \approx 1 - \alpha.
$$
...
ggplot(
  results[1:100, ],
  aes(x = rank(estimate), y = estimate, color = ok)
) +
  geom_point() +
...
Let's calculate the Bootstrap estimate of the standard error for the sample mean, one of the only estimators where we can also explicitly estimate its standard error from the data.
...
**Comment:** The Bootstrap standard error is very similar to the usual estimate. Here, both are quite far away from the *true* standard error of the mean of 49 standard exponentials $(1 / 7 \approx 0.1429)$.
...
plot_stability <- function(x) { # x is the vector of Bootstrap replications
  ...
  df$se <- sapply(df$b, function(i) sd(x[1:i])) # naive "cumsd" (could do better)

  ggplot(subset(df, b >= 20), aes(b, y = se)) +
...
plot_stability(boot) +
  ...
  geom_hline(yintercept = sd(x) / sqrt(n))
```
...
# Sample median
# Note: Population median is ln(2) / lambda = ln(2) \approx 0.693
median(x)
  ...
  system.time( # 23 minutes
    for (n in sample_sizes) {
      ...
      for (i in 1:nsim) {
        ...
        result[n, "Norm", i] <- is_ok(
          median(x) + c(-1, 1) * qnorm(0.975) * sd(boot)
        )
        result[n, "Perc", i] <- is_ok(
          quantile(boot, c(0.025, 0.975), names = FALSE)
        )
...
**Comment:** The coverage probabilities are surprisingly close to the nominal coverage of 95%, even for the tiny sample size 9. The non-parametric intervals are conservative by construction. In this example, the standard normal confidence interval are slightly anti-conservative. 

## Better CIs

At the cost of more complexity, more accurate Bootstrap confidence intervals can be constructed. One is the bias-corrected and accelerated $BC_\alpha$ confidence interval, which is an improved version of the percentile method (using different order statistics as limits). It is available in the "boot" package.
...
boot_dist <- boot(
  x,
  statistic = function(x, id) median(x[id]),
  R = 9999
)
boot.ci(boot_dist)
```
...
According to the following figure, $B = 10000$ Monte-Carlo-samples seem large enough: The p values stabilize relatively quickly.
...
result <- matrix(
  nrow = nsim,
  ncol = 3,
  dimnames = list(NULL, c("Welch", "Student", "Perm"))
)
...
if (FALSE) { # Set to TRUE to rerun
  ...
  for (i in 1:nsim) {
    ...
    result[i, "Student"] <-
      t.test(y ~ x, var.equal = TRUE)$p.value <= 0.05
  ...
  saveRDS(
    colMeans(result),
    file = paste0("simulation/", what, ".rds")
  )
}
...
# "coin" uses exact shift algorithm by Streitberg and Röhmel (1984)
library(coin)
