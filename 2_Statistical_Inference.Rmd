...
**Comments:** Based on 10'000 resamples, we estimate the real coverage probability as 93%, which is lower than the stated one (95%). This undesirable property might be a consequence of the skeweness of the data distribution. (In the plot, five of 100 confidence intervals do not cover $\mu=1$.)
...
$$
using the Bootstrap estimate of the standard error as $\hat \sigma(\hat\theta)$. 
...
The logic for applying the Bootstrap is as before: resample $B$ times with replacement, calculate the $B$ Bootstrap replications, and then use these $B$ values to calculate standard errors and/or confidence intervals.
...
statistic <- function(x, y) {
  mean_difference <- mean(y[x == "B"]) - mean(y[x == "A"])
  abs(mean_difference)
}
