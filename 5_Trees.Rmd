...
prp(fit,
  type = 2, extra = 7, shadow.col = "gray",
...
# Fit B models on B bootstrap samples and store their predictions
# on the validation data (one column per bootstrap sample)
B <- 20
...
# Very high correlations across model predictions
# (subset of three models shown)
round(cor(valid_pred[, 1:3]), 3)

# Row-wise successive mean predictions
pred_bagged <- t(
  apply(valid_pred, 1, function(z) cumsum(z) / 1:length(z))
)

# Validation RMSE per column
rmse_bagged <- apply(
  pred_bagged, 2, function(z) RMSE(valid$log_price, z)
)
...
$$
	\underbrace{\mathbb E(y_o - \hat f(x_o))^2}_{\text{Expected test squared error of } x_o} = \text{Var}(\hat f(x_o)) + \left[\text{Bias}(\hat f(x_o))\right]^2 + \underbrace{\text{Var}(\varepsilon)}_{\text{Irreducible error}},
$$
where
...
- The model fills about 125 MB of RAM. This is a lot, given the fact that the diamonds data is not too large.
...
imp <- imp / sum(imp)
barplot(imp, horiz = TRUE, col = "chartreuse4", las = 2)
```
...
To illustrate the boosting process, we will try to approximate a (univariate) sine function with 1, 2, 10, 100, and 1000 boosted tree *stumps* (single-split trees).
...
# Boosted tree stumps. Or use deeper trees for faster convergence
fit <- xgb.train(
  params = list(learning_rate = 1, max_depth = 1),
  data = xgb.DMatrix(cbind(x = x_train), label = y_train),
  nrounds = 1000
)

# Get predictions for different number of rounds
df_plot <- data.frame(x = x, Truth = y)
N <- c(1, 2, 10, 100, 1000)
for (n in N) {
  df_plot[as.character(n)] <-
    predict(fit, cbind(x = x), iterationrange = c(1, n))
}

# Transpose for plotting
df_plot_long <- df_plot |>
  pivot_longer(-x, values_to = "Prediction") |>
  mutate(name = factor(name, levels = c(N, "Truth")))

ggplot(df_plot_long, aes(x = x, y = Prediction)) +
  geom_line(color = "chartreuse4") +
  facet_wrap("name")
```

**Comments:** The approximation works very well, but only within the data range used for training, i.e., in $[0, 3\pi/2]$.
...
# Add trees until CV validation MSE stops improving over
# last 20 rounds
cvm <- xgb.cv(
  params = params,
  data = dtrain,
...
# Fit model on full training data using optimal number of rounds
fit <- xgb.train(
  params = params,
  data = dtrain,
  print_every_n = 50,
...
# Test performance
rmse(y_test, predict(fit, data.matrix(X_test))) # 0.099
```
...
```{r}
# This code is pretty long. It may serve as a general template to fit
# a high-performance XGBoost model
library(ggplot2)
library(withr)
library(xgboost)
library(MetricsWeighted)
...
if (tune) {
  # Use default parameters to set learning rate with suitable
  # number of rounds
  params <- list(
    objective = "reg:squarederror",
    learning_rate = 0.1
  ...
  cvm # -> a lr of 0.1 provides ~200 trees, which seems convenient
  ...
  # Loop over grid and fit XGBoost with 5-fold CV and early stopping
...
rmse(y_test, pred) # 0.099
r_squared(y_test, pred, reference_mean = mean(y_train)) # 0.9905
...
if (tune) {
  ...
  params <- list(
    learning_rate = 1,
    objective = "mse",
    num_threads = 7 # choose number of threads
  )
...
rmse(y_test, pred) # 0.3012
r_squared(y_test, pred, reference_mean = mean(y_train))  # 82.4%
...
- The effect of the hour of day does not seem intuitive.
...
5. In the gradient boosted trees algorithm for the squared error, why do we fit a regression tree to the residuals instead of simply adding as new model the residuals?
