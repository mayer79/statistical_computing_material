...
plot(history, metrics = "root_mean_squared_error", smooth = FALSE) +
  coord_cartesian(
    xlim = c(2, length(history$metrics$loss)), ylim = c(0.1, 0.2)
  )
...
# Function that maps data.frame to network input (a list with
# a scaled matrix and an integer vector representing pu_loc)
...
emb_pu_loc <- input_pu_loc |>
  layer_embedding(
    input_dim = nlevels(train$pu_loc) + 1,
    output_dim = 10
  ) |>
  layer_flatten()
...
if (FALSE) { # Set to true to refit
  ...
  save_model_weights(nn, "taxi/taxi_nn.h5")
} else {
  load_model_weights(nn, "taxi/taxi_nn.h5")
}
...
# Permutation importance regarding MSE
perm_importance(
  nn,
  X = X_test,
  y = y_test,
...
Let's apply above scheme for diamond prices. We use the {effectplots} package to simplify the process.
...
average_observed(diamonds[xvars], diamonds$price) |>
  plot()
...
# Permutation importance wrt test MSE
imp <- perm_importance(
  fit,
  X = diamonds[-ix, xvars],
  y = diamonds$price[-ix],
...
Here a short [comparison](https://github.com/mayer79/ML_Algorithm_Comparison) of some properties of the ML algorithms learned in this lecture (screenshot as per May 2025): 
