...
- The amount of overfitting (or better: optimism) decreases for growing $k$, which makes sense.
...
head(paramGrid[order(paramGrid$RMSE), ], 4)

ggplot(paramGrid, aes(x = k, y = RMSE)) +
  geom_point(color = "chartreuse4") +
  geom_line(color = "chartreuse4") +
  geom_errorbar(
    aes(ymin = RMSE - SE, ymax = RMSE + SE),
    width = 0
  ) +
  ggtitle(
...
system.time( # 5s
  fit <- h2o.glm(
    ...
    lambda_search = TRUE,
    alpha = 0 # ratio of L1 to L2 penalty strength (0 = no L1)
  )
)
fit # Validation R2: 78.2%; best lambda essentially 0 (no penalty)
...
For small data sets, the raw data is usually loaded into R/Python and then preprocessed in memory. For large data sets, this becomes slow or even impossible. One option is to externalize the preprocessing to a database server, or use big data technology like Spark that work out-of-core. In this section, we take a short trip into that world.
...
We will introduce SQL with examples written in the in-process database system [DuckDB](duckdb.org). "In-process" means that when DuckDB is run from R/Python, it is embedded in the R/Python process itself. DuckDB is convenient for several reasons:

- It is easy to install.
- It has no external dependencies.
- It is extremely fast.
- It supports working with csv and Parquet files on disk as well as with R/Python tables in RAM.
- It plays well together with Apache Arrow.
- It works out-of-core, i.e., when data does not fit into RAM.

DuckDB has been released in 2019 as an open-source project written in C++. Later, we will also take a look at Spark, the famous big data technology.
...
# Some SQL implementations also offer "Window" functions
query <- "
...
WITH counts AS (
  SELECT
    PULocationID,
    trip_distance,
    DATE_DIFF('minutes', tpep_pickup_datetime, tpep_dropoff_datetime) AS duration,
    tpep_pickup_datetime,
    COUNT(*) OVER (PARTITION BY PULocationID) AS N
  FROM 'taxi/yellow_tripdata_2018-01.parquet'
)
SELECT
  LOG(trip_distance) AS log_distance,
  LOG(duration) AS log_duration,
  DAYNAME(tpep_pickup_datetime) AS weekday,
  EXTRACT(hour FROM tpep_pickup_datetime) AS pu_hour,
  CASE WHEN N >= 100000 THEN CAST(PULocationID AS varchar) ELSE 'Other' END AS pu_loc
FROM counts
WHERE trip_distance BETWEEN 0.2 AND 100
...
Let's start a local Spark instance and apply some small queries to the diamond data. We will use both SQL and "dplyr" code, which will then be translated into Spark SQL.
...
query <- "
  SELECT
    color
    , price
    , AVG(price) OVER (PARTITION BY color) AS avg_price
  FROM diamonds
  LIMIT 2
"
dbGetQuery(sc, query)
...
df <- spark_read_parquet(
  sc,
  name = "taxi",
  path = "taxi/yellow_tripdata_2018-01.parquet"
)
