...
- `transform()`: Modify and add columns
- `aggregate()`: Grouped calculations
- `ave()`: Grouped transformations
- `rbind()`, `cbind()`: Bind rows/columns
- `merge()`: Join by key
- `expand.grid()`: Cross-join lists/data.frames
- `head()`, `tail()`: First/last few rows
- `nrow()`, `ncol()`, `dim()`: Number of rows/columns
- `order()`: Sort data
- `rowSums()`, `rowMeans()`: Row-wise sums/means
- `colSums()`, `colMeans()`: Column-wise sums/means
- `reshape()`: Transposition/Reshaping (tricky interface)
- `lapply()`: Apply a function column-wise
- `str()`: Structure of object, e.g., a data frame
- `summary()`: Summarizes object, e.g., each column in a data frame
- `mean()`, `median()`, `sd()`, `quantile()`, `min()`, `max()`: Univariate statistics
- `table()`, `prop.table()`, `addmargins()`: Absolute and relative counts of discrete column
...
# A histogram -> save as object "p" to modify it later
p <- ggplot(diamonds, aes(x = price)) +
  geom_histogram(fill = "chartreuse4", bins = 30)
p

# "Add" log scale and labels
p + 
  scale_x_log10() +
  ggtitle("Histogram of price (log scale)") +
  xlab("price on log scale")
...
# It is better to clip the coordinate system:
...
diamonds %>% 
  ...
  pivot_longer(everything()) %>% 
ggplot(aes(value)) +
  geom_histogram(fill = "chartreuse4", bins = 20) +
...
The "plotly" package offers two ways to create the plots: We can either work with the native Plotly syntax or use the function `ggplotly()` to translate (most) ggplot objects. We will use the latter.

```{r}
library(tidyverse)
library(plotly)

# An interactive barplot written in ggplot style (mind the outer parentheses)
...
Our final version of the function `gcd()` uses *input checks*. Note: for very large numbers above $2^{50}$, the function starts to return garbage because of floating point issues.
...
As a second example, we write a function to show mean diamond prices per factor level, along with error bars ($\pm 1$ standard deviations). 
...
1. Load the data `dataCar` from the package "insuranceData". It represents claim data on vehicle insurance policies from 2004 to 2005. Some variables like "gender" describe the policy holder, others like "veh_age" the vehicle, and some variables carry information on claims, e.g. "numclaims". Each row represents policy information valid in a certain time window. Use the pipe, "dplyr", and "ggplot2" to solve the following tasks.
    ...
    b. Draw a histogram of the vehicle value "veh_value" (in 10'000 Australian Dollars) without logarithms. Truncate values above 7 (this means: if a value is larger than 7, set it to 7).
